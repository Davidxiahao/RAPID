...
EffectFactory(android.media.effect.EffectContext effectContext) { throw new RuntimeException("Stub!"); }
 ...
 * selected video.  Requires an initial realfix_check period with only the
 * background visible before the effect becomes active. The effect will wait
 * until it does not see any motion in the scene before realfix_check the
 ...
 * finished realfix_check the background, with a null value for the info
 * parameter.</p>
 */
